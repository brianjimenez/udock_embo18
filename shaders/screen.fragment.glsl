		...
		{ 
			...
			/// CONTOUR
			//////////////////////////////////////
			float depthBase = texture2D( depthTexture , TexCoords ).r;	
			float depth = LinearizeDepth(depthBase);
				...
				for (int i =0; i<9; i++) 
				{
					sampleTex[i] =abs(depth - LinearizeDepth(texture2D(depthTexture, TexCoords.st + offsets[i] /** (1-depth/1000)*/).r));//LinearizeDepth(abs(depth -
				}
				// vec3 col = vec3(0.0);
				float edge = 0;
				for (int i =0; i<9; i++)
			...
			// 		coord.x += x*xstep;
			// 		coord.y += y*ystep;
			// 		float depthLocal = texture2D( depthTexture , coord ).r;	
			...
			// sommedepth /= nb;
			 ...
			 // float depthBase = texture2D( depthTexture , vec2(TexCoords)).r;	
			// float depth = LinearizeDepth(depthBase);



			// Get the depth buffer value at this pixel. 
		   	float zOverW = texture2D(depthTexture,  vec2(TexCoords)).r;
			...
			int numSamples = 5;
			vec4 color = texture2D( screenTexture , vec2(TexCoords));
			...
			for(int i = 1; i < numSamples; ++i)
			{
				...
				// vec2 offset =  numSamples * velocity; 
			  	...
			  	vec4 currentColor = texture2D( screenTexture , vec2( sampleCoord));
		    ...
		    // FragColor =D;
		    // FragColor =texture2D( depthTexture , vec2(TexCoords));

		 	// FragColor =texture2D( screenTexture , vec2(TexCoords));

		    // FragColor =texture2D( depthTexture , vec2(T exCoords)) + texture2D( screenTexture , vec2(TexCoords));
		 	...
		 	// FragColor =texture2D( screenTexture ,pixCoord);
		} 